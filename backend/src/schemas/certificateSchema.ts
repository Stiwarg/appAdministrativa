import { z } from 'zod';
import { Certificate } from '../utils/enums';
import { month, bimesters } from '../utils/constantes';


// Función para obtener los periodos según el tipo de archivo
const getValidPeriods = ( typeFile : Certificate ): string[] => {
    if ( typeFile === Certificate.CERTIFICADO_RETENCION_IVA || typeFile === Certificate.CERTIFICADO_INDUSTRIA_COMERCIO ) return ["Todos",  ...bimesters ];
    if ( typeFile === Certificate.CERTIFICADO_RETENCION_FUENTE_RTE ) return ["Todos", ...month ]; 
    return [];    
};

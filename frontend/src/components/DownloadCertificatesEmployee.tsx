...
const DownloadCertificatesFormEmployee: React.FC = () => {
    ...
    const [user, setUser] = useState<null | { nit: number, rolId: number }>(null);
    ...
    useEffect(() => {
      api.get("/protected", { withCredentials: true })
        .then(( response ) => {
          logInfo("✅ Usuario autenticado", response.data.user.nit );
    ...
    const handleSearch = async ( data: certificateRequest ) => {
      ...
      const requestData = { 
        ...data, 
        nit: user?.rolId === 2 ? user.nit : data.nit
      };
      logInfo('Esta es la busqueda completa:', requestData );
      try {
        const response = await api.post('/buscarDetallesUsuario', requestData , {
    ...
    const handleDownloadCertificate = async ( data: certificateRequest ) => {
      ...
      try {
        const response = await api.post('/certificado', requestData, {
          withCredentials: true,
          responseType: "blob"
        });
                    ...
                    <div>
                      ...
                      { certificateFound ? (
                        <div className="p-6 bg-green-50 border border-green-200 rounded-lg">
                          ...
                          onClick={() => handleDownloadCertificate({ nit: user!.nit, period: selectedPeriod, typeFile: selectedTypeCertificate, year: selectedYear! })}
                          >
...
export default DownloadCertificatesFormEmployee;